...
describe('classyHeader: renders DOM headers of different levels and classes', () => {
  ...
  describe('an H1 with text: "good bye", and two classes', () => {
    ...
    it('have class: "fancy"', () => {
      const hasclassfancy=actual.classList.contains('fancy');
      expect(hasclassfancy).toEqual(true);
    });
    it('have class: "hover-right"', () => {
      const hasclasshoverright=actual.classList.contains("hover-right");
      expect(hasclasshoverright).toEqual(true);;
    });
    it('has childElementCount: 0', () => {
      expect(actual.childElementCount).toEqual(0);
    });
